...
DEFAULTS = {
    ...
    # 빠른 학습 — 체크(기본) 시 원본(915×1060급)을 학습 전에 256px 로 축소한
    # 캐시로 학습해 빠르다. 해제 시 캐시 없이 원본 그대로 학습한다 (느림).
    "fast_train":       (tk.BooleanVar, True),
...
class LabApp(tk.Tk):
    ...
    def _build_left(self, p):
        ...
        ttk.Label(m, text="빠른 학습 — 체크 시 원본(915×1060급)을 학습 전에 "
                          "256px 로 축소(캐시)해 빠르게 학습합니다. 해제 시 "
                          "원본 이미지를 그대로 사용합니다 (품질 우선 · 느림).",
