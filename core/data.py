...
def build_multihead_data(cfg, log=None):
    ...
    # 사전 리사이즈 캐시 (기본 활성). img_size 가 캐시보다 크면 원본을 쓴다.
    cache_size = int(cfg.get("cache_size", 256))
    cache_dir = None
    if bool(cfg.get("image_cache", True)) and img_size <= cache_size:
        cache_dir = data_root / "_cache" / f"{crop_mode}_{cache_size}"
        log(f"이미지 캐시 사용: {cache_dir} (최초 epoch 에 {cache_size}px 로 생성, "
            f"이후 재사용)")
