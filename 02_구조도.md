...
| 계층 | 역할 | 없으면 |
|---|---|---|
| ① 실행 | 실험이 돌아갑니다 | 아무것도 안 됨 |
| ② 기록 | 실험이 **의미로 남습니다** | run 30개가 숫자 더미가 됨 |
| ③ 자료 | 실험이 **보여줄 수 있는 것으로 남습니다** | 자료 만들 때 스크린샷 노가다 |

**의존 방향은 아래에서 위로 단방향입니다.** 학습 코드는 report가 생성되는 것을 모르고, report 생성기는 그림 생성기를 모릅니다. 각각 run 디렉터리의 **파일만** 읽고 씁니다.

> **이 무의존 구조의 실익** — 그림 생성이 실패해도 학습 결과는 남습니다. matplotlib 오류로 40분짜리 학습이 무효가 되는 일이 없습니다.
...
**GUI는 학습 로직을 소유하지 않습니다.** 화면 값을 config로 쓰고, 별도 프로세스를 띄우고, 결과 파일을 읽어 보여줄 뿐입니다. 따라서 GUI 없이 터미널에서 동일 실험이 그대로 재현됩니다.
...
│  ├─ check_env.py               #   설치 환경 확인 (torch·CUDA·Tkinter·폰트)
...
`tasks/cifar10.py`를 남겨두는 것은 **"같은 하네스로 두 과제가 돌아간다"는 증거**이자, 회귀 테스트의 픽스처입니다.

---

## 4. 모델 레지스트리 — 모델 카드 방식

**모델 교체를 유연하게 만드는 핵심 설계.** 레지스트리 항목을 생성자 함수가 아니라 **명세 객체**로 둡니다. 학습·CAM·그림·리포트가 모두 이 객체만 읽으므로, 새 모델을 추가할 때 **다른 파일을 전혀 건드리지 않습니다.**
...
| `simple_cnn` | **픽스처 (실험 금지)** | 0.1M | 32 | 1e-3 | 128 / 256 |

> **`simple_cnn` 은 회귀 검증 전용입니다.** 사전학습 가중치가 없고(`build=lambda pre: None`)
> 동결할 백본이 없어 `freeze_epochs` 가 무시되므로, 같은 config 를 줘도 혼자 다른 조건으로
> 학습됩니다. 비교표에 넣으면 공정성이 깨집니다 — 근거는 `01_기획서.md` §4.1.
>
> 코드가 이 규칙을 강제합니다.
>
> | 지점 | 동작 |
> |---|---|
> | `ModelSpec.is_fixture` | `True` |
> | `arch_choices(include_fixture=False)` | 기본 목록에서 제외 (`tasks/models_registry.py`) |
> | `build_backbone()` | 호출 시 `ValueError` — `build_simple_cnn` 경로로만 생성 (`core/models.py`) |
> | `lab_gui.py` | 전용 파라미터 프레임은 `arch=simple_cnn` 일 때만 노출, Study 모드에서는 숨김 |
>
> 단 `build_multihead_model()` 은 simple_cnn 도 `build_simple_cnn_features()` 로 받아
> crop/stage 멀티헤드를 정상 구성합니다. **멀티헤드가 안 되는 것이 아니라, 조건이 안 맞는 것입니다.**

> ConvNeXt의 lr 범위가 낮은 것은 의도적입니다. Transformer 계열 학습 방식을 따르는 모델이라 큰 lr에서 발산하기 쉽습니다. 레지스트리에 담아두면 사용자가 이를 몰라도 됩니다.
...
**다른 파일은 수정하지 않습니다.** 이것이 축 A의 실질입니다.

### 4.3 Grad-CAM을 레지스트리에 넣는 이유

계열마다 CAM 대상 레이어와 후처리가 다릅니다.
...
이 차이를 `CamSpec`에 담아두면 **그림 생성 코드는 arch를 몰라도 됩니다.** 새 모델을 추가할 때 CAM 코드를 고치지 않습니다.
...
> 데이터 구조·JSON 스키마·함정의 상세는 **`04_데이터_명세서.md`**. 여기서는 코드 구조만 다룹니다.
...
| 단계 | 규칙 |
|---|---|
| **짝 검증** | `labeled`·`source`의 묶음 번호가 다르면 **중단**. 교집합 0건이 조용히 지나가는 것을 막습니다 |
| 추출 | `data/_extracted/`. 이미 추출된 아카이브는 건너뜀. tar 직접 읽기는 랜덤 액세스가 느려 채택하지 않음 |
| 조인 | JSON ↔ JPG를 **파일명 stem**으로. JSON의 `fname` 필드는 실제 파일명이 아니므로 쓰지 않습니다 |
| 인덱스 | **학습은 `index.csv`만 읽습니다.** 원본 JSON을 직접 파싱하지 않음 |
...
| 제약 | 이유 |
|---|---|
| 그룹 분할이 필요합니다 | 한 개체를 한 달간 27~40장 연속 촬영 → 같은 개체가 양쪽에 섞이면 성능이 크게 과대평가됩니다 |
| 층화 분할이 필요합니다 | **한 아카이브에 한 단계만 있는 경우가 실제로 존재** → 그룹 단위로만 자르면 단계 균형이 무너집니다 |

`GroupShuffleSplit`는 층화를 못 하고, `train_test_split(stratify=…)`는 그룹을 못 지킵니다. **둘을 동시에 처리하는 것은 `StratifiedGroupKFold` 뿐입니다.**
...
│
├─ study_02_backbone/                # 번호는 GUI 가 자동 부여 (study_NN)
│  ├─ STUDY.md                      # 비교 결론
│  ├─ index.csv                     # 이 study 의 멤버 카탈로그
│  ├─ split.json                    # 고정된 분할 (공정성)
│  ├─ figures/                      # study 단위 비교 그림 4종
│  │  ├─ cost_accuracy.png          #   비용 vs 정확도 산점도
│  │  ├─ loss_overlay.png
│  │  ├─ runs_parallel.png
│  │  └─ metric_progress.png
│  └─ study_02_backbone__arch-resnet18/   # ★ 멤버 run — 단일 run 과 같은 구조
│     ├─ config.json · metrics.json · best.pt · logits_*.npy
│     ├─ report.md                  #   모델별 리포트 (단일 실행과 동일)
│     └─ figures/                   #   모델별 그림 8종
│
└─ exp_260815_jk_07/
...
| # | 계약 | 위반 시 |
|---|---|---|
| **C1** | 모든 설정을 `--config` 하나로 받습니다 (하드코딩 금지) | GUI가 제어할 수 없는 값이 생김 |
| **C2** | 모든 출력을 `--out` 아래에만 씁니다 | run 간 결과 덮어쓰기 |
| **C3** | epoch마다 JSON 한 줄을 `flush=True`로 출력 | 로그가 버퍼에 갇혀 진행이 안 보임 |
| **C4** | 종료 시 `metrics.json`·`logits_*.npy`·`labels.npy`·`status.json`을 남깁니다 | 결과를 화면에 못 띄움 |
| **C5** | 그림·리포트를 생성하지 않습니다 | 그림 실패가 학습을 무효화함 |

### 7.2 로그 프로토콜

사람이 읽는 로그를 정규식으로 파싱하지 않습니다. 기계용 JSON 라인을 별도로 냅니다.
...
> **판단하지 않고 사실만 짚습니다.** 자동 규칙이 "이렇게 하세요"까지 말하면 사람이 생각을 멈춥니다. 해석(§7)은 사람의 몫입니다.

### 8.2 갱신 방식

사람이 결론을 쓰면 **해당 섹션만** 갱신합니다. 앵커로 구간을 나눠 자동 생성부와 사람 작성부가 서로를 덮어쓰지 않게 합니다.
...
두 경로는 **같은 계산 결과를 공유**하고 렌더러만 다릅니다. 지표 계산 코드는 한 벌입니다.
...
**초기 노출 제한** — 처음 보이는 조작 대상은 7개 이하로 유지합니다. 나머지는 `고급` 아코디언에 격리.

**threshold 즉시 반응의 조건** — `logits_{head}.npy`를 최초 1회 메모리에 로드하고, 이후 threshold 변경은 numpy 마스크 재집계로만 처리합니다. 모델 재추론이 개입하는 순간 즉시성이 붕괴합니다.
...
### 11.1 멤버별 리포트는 누가 만드나

`run_study.py` 는 **그림·리포트를 만들지 않습니다** — 워커 계약 C5 를 지키기 위해서입니다.
멤버 run 의 `report.md`·`figures/` 는 콘솔(`lab_gui.py`)이 학습과 분리된 프로세스로 만듭니다.

| 시점 | 하는 일 |
|---|---|
| **모델 하나가 끝날 때마다** | 그 멤버의 그림·리포트를 만듭니다. 다음 모델이 학습하는 동안 돌므로 대기 시간이 없습니다 |
| **Study 전체가 끝난 뒤** | 위에서 놓친 멤버(재개로 건너뛴 run)를 보충하고, `index.csv` 를 갱신한 뒤 비교 그림을 만듭니다 |

후처리는 **워커 스레드 1개로 직렬화**합니다. 다음 모델이 이미 GPU 를 쓰는 중이라 동시에
여러 개를 돌리면 메모리만 축납니다. 실패해도 로그만 남기고 넘어갑니다 — 학습 결과는 이미
파일로 저장돼 있습니다.

터미널에서 `run_study.py` 를 직접 쓴 경우에는 멤버 리포트가 생기지 않습니다. 필요하면
멤버마다 아래를 돌립니다.

```bash
python make_figures.py --run runs/study_02/study_02__arch-resnet18
python make_report.py  --run runs/study_02/study_02__arch-resnet18
```
...
> 탐지·분할은 보유 데이터에 bbox 어노테이션이 있어 경로가 열려 있지만, **지금 착수하지 않습니다.** 분류가 안정된 뒤 재검토합니다.
