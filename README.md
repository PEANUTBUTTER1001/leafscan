...
| 빠른 학습 | 입력 해상도 옆 체크박스. 원본을 256px 축소 캐시로 학습(기본 체크) ↔ 원본 그대로 학습. 아래 상세 |
...
| 상태 | 동작 | 언제 쓰나 |
|---|---|---|
| ✔ **체크 (기본)** | 원본(915×1060급)을 **최초 epoch 에 256px 로 축소해 캐시**(`data/_cache/`)하고 이후 재사용 | 평상시 실험 전부. 원본 JPEG 디코딩이 epoch 시간의 병목이라 훨씬 빠름 |
| ☐ 해제 | 캐시 없이 **원본 이미지를 매번 그대로 디코딩**해 학습 | 품질을 최우선으로 확인하고 싶은 최종 검증 run |
